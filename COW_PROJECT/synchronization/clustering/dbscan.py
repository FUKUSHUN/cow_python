...
class CommunityDBSCAN:
    dbscan = None

    def __init__(self, eps, minPts):
        self.dbscan = DBSCAN(eps, minPts)

    def create_community(self, X, cows_list):
        """ DBSCANを用いたコミュニティを作る """
        cluster = self.dbscan.fit(X)
        communities = self._make_nodes(cows_list, cluster)
        return communities

    def _make_nodes(self, cows_list, cluster):
        """ コミュニティごとに同じコミュニティのメンバの個体番号をリスト化したものを作成する """
        node_list = []
        # ノイズ以外のクラスタごとに牛の個体番号を格納する
        n_max = int(max(cluster))
        for i in range(1, n_max+1):
            node_list.append([cows_list[j] for j in range(len(cluster)) if cluster[j] == i])
        # ノイズは個別のクラスタに分ける
        for i, cow_id in zip(cluster, cows_list):
            if (i == 0):
                node_list.append([cow_id])
        return node_list
