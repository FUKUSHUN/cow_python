        ...
        for cow_id in target_list:
            if (cow_id in cow_id_list):
                ...
                for (start_point, end_point) in start_end_list:
                    try:
                        community_series, graph_series = _extract_community(t_list, communities_list, community_graph_list, start_point, end_point)
                        pdb.set_trace()
                        ...
                        feature_list.append([start_point, ave_dence, features[2], features[0]*features[5]])
                    except KeyError:
                        pdb.set_trace()
        e2 = time.time()
