...
def init(dt):
    ...
    os.mkdir("serial")
    #存在リストの確認
    cow_list = [20112, 20113, 20116, 20117, 20120, 20122, 20126, 20129, 20131, 20163, 20170, 20171, 20192, 20199, 20215, 20256, 20261, 20264, 20267, 20268, 20288, 20289, 20295]
    #各牛に列番号を割り振り
    dic = {}
...
def main(start, end, dic):
    print(dic)
    filename = "for_web/weight_rssi.csv"
    ...
    while(dt < end):
        ...
        while(a < dt and a < end):
            ...
            data_list.append([b.strftime("%Y/%m/%d %H:%M:%S")] + v_array.tolist())
            del df
            gc.collect()
...
if __name__ == '__main__':
    ...
    end = datetime.datetime(2018, 10, 25, 14, 0, 0)
    s = time.time()
    dic = init(end)
    main(start, end, dic)
    e = time.time()
