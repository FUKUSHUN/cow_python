...
def semi_supervised_learning(model, train_dataset, test_dataset, candidate_dataset, target_labelname, num):
    """
    半教師あり学習のメインルーチン
    Parameter
    ...
    for i in range(num):
        # 既存のモデルで予測を行い信頼度を算出
        candidate_dataset = predict_probability(model, candidate_dataset) # 予測ラベルと予測確信度を列に追加
        ...
        print(i+1, "回目: ", score)
        # 次の予測データを作成する
        df = df.drop(["Probability", "Prediction"], axis=1) # 新たに追加した行の削除
...
if __name__ == '__main__':
    ...
    rf1 = semi_supervised_learning(rf1, train_dataset1, test_dataset1, candidate_dataset, "Target1", 20) # 半教師あり学習の中心的処理を行う
    rf2 = semi_supervised_learning(rf2, train_dataset2, test_dataset2, candidate_dataset, "Target2", 20) # 半教師あり学習の中心的処理を行う
