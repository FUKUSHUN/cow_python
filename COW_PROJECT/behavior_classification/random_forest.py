...
model1 = RandomForestClassifier(random_state=0, max_depth=3, n_estimators=7) # seedの設定。seedを設定しないとモデルが毎回変わるので注意
model1 = model1.fit(W_train, Z_train)
prediction = model1.predict(X_test)
evaluation1 = accuracy_score(prediction, Y_test)

model2 = RandomForestClassifier(random_state=0, max_depth=3, n_estimators=7) # seedの設定。seedを設定しないとモデルが毎回変わるので注意
